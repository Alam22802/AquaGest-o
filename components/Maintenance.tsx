...
const Maintenance: React.FC<Props> = ({ state, onUpdate, currentUser }) => {
  ...
  const selectAllCages = () => {
    const visibleIds = filteredCages.map(c => c.id);
  ...
  const filteredCages = useMemo(() => {
    return state.cages.filter(c => {
      ...
      if (filterBatchId === 'none') {
        ...
        const activeInBatch = c.batchId === filterBatchId;
        const harvestedFromBatchIds = new Set(
          (state.harvestLogs || [])
            .filter(h => h.batchId === filterBatchId)
            .map(h => h.cageId)
        );
        const recentlyHarvested = !c.batchId && harvestedFromBatchIds.has(c.id) && (c.status === 'Limpeza' || c.status === 'Manutenção');
        if (!activeInBatch && !recentlyHarvested) {
          return false;
        }
      }
  ...
  }, [state.cages, state.harvestLogs, filterStatus, filterModel, filterBatchId]);
          ...
          <div className="lg:col-span-2 space-y-6">
            ...
            <div className="bg-white p-4 rounded-3xl border border-slate-200 shadow-sm space-y-3">
              <div className="flex items-center justify-between">
                ...
                {(filterStatus || filterModel || filterBatchId) && (
                  <button 
                    onClick={() => {
                      setFilterStatus('');
                      setFilterModel('');
              ...
              <div className="flex flex-wrap items-center gap-2">
                {[
                  { id: '', label: 'Todas', color: 'bg-slate-100 text-slate-700 hover:bg-slate-200' },
                  { id: 'Limpeza', label: 'Limpeza', color: 'bg-amber-50 text-amber-700 border-amber-200 hover:bg-amber-100' },
                  { id: 'Avaliação', label: 'Avaliação', color: 'bg-purple-50 text-purple-700 border-purple-200 hover:bg-purple-100' },
                  { id: 'Manutenção', label: 'Manutenção', color: 'bg-red-50 text-red-700 border-red-200 hover:bg-red-100' },
                  { id: 'Disponível', label: 'Disponível', color: 'bg-emerald-50 text-emerald-700 border-emerald-200 hover:bg-emerald-100' },
                  { id: 'Ocupada', label: 'Ocupada', color: 'bg-blue-50 text-blue-700 border-blue-200 hover:bg-blue-100' },
                  { id: 'Sucata', label: 'Sucata', color: 'bg-slate-200 text-slate-700 border-slate-300 hover:bg-slate-300' },
                ].map(chip => {
                  ...
                  return (
                    <button
                      ...
                      type="button"
                      onClick={() => {
                        setFilterStatus(filterStatus === chip.id ? '' : chip.id);
                        setSelectedCageIds([]);
                      }}
                    ...
                    >
                      ...
                      <span className={`px-1.5 py-0.5 rounded-md text-[9px] font-black ${
                        isActive ? 'bg-white/20 text-white' : 'bg-black/5 text-slate-700'
                      }`}>
                        {count}
                      </span>
                    </button>
            ...
            {/* Filter Dropdowns and Actions Bar */}
            <div className="flex flex-wrap items-center justify-between gap-3 bg-slate-50 p-3 rounded-2xl border border-slate-200">
              <div className="flex flex-wrap items-center gap-2 flex-1">
                {/* Select Status Dropdown */}
                <select 
                  className="px-3 py-2 bg-white text-slate-700 rounded-xl text-[10px] font-black uppercase tracking-widest outline-none border border-slate-200 focus:ring-2 focus:ring-red-500"
                  value={filterStatus}
                  onChange={(e) => {
                ...
                >
                  <option value="">Todos os Status ({state.cages.length})</option>
                  <option value="Limpeza">Limpeza ({statusCounts['Limpeza'] || 0})</option>
                  <option value="Avaliação">Avaliação ({statusCounts['Avaliação'] || 0})</option>
                  <option value="Manutenção">Manutenção ({statusCounts['Manutenção'] || 0})</option>
                  <option value="Disponível">Disponível ({statusCounts['Disponível'] || 0})</option>
                  <option value="Ocupada">Ocupada ({statusCounts['Ocupada'] || 0})</option>
                  <option value="Sucata">Sucata ({statusCounts['Sucata'] || 0})</option>
                </select>
              ...
              <div className="flex items-center gap-2">
                <button 
                  ...
                  className="px-4 py-2 bg-white hover:bg-slate-100 text-slate-700 border border-slate-200 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all shadow-sm"
                >
                  {(() => {
                    const visibleIds = filteredCages.map(c => c.id);
                    const allVisibleSelected = visibleIds.length > 0 && visibleIds.every(id => selectedCageIds.includes(id));
                    return allVisibleSelected ? 'Desmarcar Visíveis' : `Selecionar Visíveis (${filteredCages.length})`;
                  })()}
                </button>
              </div>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              {filteredCages.length === 0 ? (
                <div className="col-span-full bg-white p-12 rounded-3xl border border-dashed border-slate-200 text-center flex flex-col items-center justify-center gap-3">
                  <AlertTriangle className="w-10 h-10 text-amber-500" />
                  <h4 className="text-sm font-black uppercase text-slate-700">Nenhuma gaiola encontrada</h4>
                  <p className="text-xs text-slate-400 font-bold uppercase">Nenhuma gaiola corresponde aos filtros definidos.</p>
                  {(filterStatus || filterModel || filterBatchId) && (
                    <button 
                      onClick={() => {
                        setFilterStatus('');
                      ...
                      className="mt-2 px-4 py-2 bg-slate-900 text-white rounded-xl text-xs font-black uppercase tracking-widest hover:bg-slate-800 transition-all"
                    >
                      Limpar Filtros
                    </button>
                  )}
