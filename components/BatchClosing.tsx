...
const isBatchMatch = (targetIdOrName: string | undefined, batch: any): boolean => {
...
const checkIsFeedingLogForBatch = (f: any, batch: any, harvestLogs: any[] = [], cages: any[] = [], allBatches: any[] = []) => {
...
const BatchClosing: React.FC<Props> = ({ state, onUpdate, currentUser }) => {
  ...
  const batches = useMemo(() => {
    ...
    return (state.batches || [])
      .filter(batch => {
        ...
        const activeUncoveredCages = (state.cages || []).filter(c => 
          c.batchId === batch.id && 
          c.status === 'Ocupada' && 
  ...
  const batchData = useMemo(() => {
    ...
    const slaughteredReceptionWeight = slaughters.reduce((acc, curr) => acc + (curr.receptionWeight || 0), 0);
    const slaughteredWeight = slaughters.reduce((acc, curr) => acc + (curr.netWeight || curr.receptionWeight || 0), 0);
    ...
    // Calculate feed prices per kg based on feed stock entries or default price
    ...
    // Calculate total feed cost and breakdown by feed model for this batch
    const feedDetailsByType: { feedTypeId: string; typeName: string; amountKg: number; cost: number; pricePerKg: number }[] = [];
    let totalFeedCost = 0;

    Object.entries(feedingByType).forEach(([typeName, amountGrams]) => {
      const feedType = (state.feedTypes || []).find(t => t.name === typeName);
      ...
      feedDetailsByType.push({
        feedTypeId: feedType?.id || typeName,
        typeName,
        amountKg,
        cost,
        pricePerKg
    ...
    const slaughteredTotalRevenue = slaughters.reduce((acc, s) => {
    ...
    const feedVirtualEntries = feedDetailsByType.map((fe, idx) => ({
      id: `feed-expense-virtual-${fe.feedTypeId}-${idx}`,
      batchId: batch.id,
      description: `Consumo de Ração - ${fe.typeName} (${formatNumber(fe.amountKg, 1)}kg x R$ ${formatNumber(fe.pricePerKg, 2)}/kg)`,
    ...
    const slaughterVirtualEntries = slaughters.map((s, idx) => {
      const unitP = harvestsByBatch[0]?.unitPrice || (s.invoiceValue && s.packedQuantity ? s.invoiceValue / s.packedQuantity : 0);
      const estimatedVal = s.invoiceValue && s.invoiceValue > 0 
        ? s.invoiceValue 
        : ((s.receptionWeight || 0) * unitP);
        
      return {
        id: `slaughter-virtual-${s.id || idx}`,
        batchId: batch.id,
    ...
    // Accuracy (Assertividade): Real Delivered/Reception Weight / Expected Weight
  ...
  const handleSaveAdjustFeed = () => {
    ...
    const otherLogs = (state.feedingLogs || []).filter(f => {
      const isForThisBatch = checkIsFeedingLogForBatch(f, currentBatch, state.harvestLogs, state.cages, state.batches);
      if (isForThisBatch) {
        ...
        <div className="space-y-8">
          ...
          <div className="space-y-8">
              ...
              <div className="bg-white p-8 rounded-[2.5rem] shadow-sm border border-slate-200 space-y-6 print-card print-no-break">
                ...
                <div className="grid space-y-3 print-grid-3">
                  <div className="p-4 bg-slate-50 rounded-2xl border border-slate-100 print:bg-slate-50">
                    ...
                    <span className="text-xl font-black text-slate-800 italic print-text-xl">{formatNumber(batchData.totalReceptionWeight, 1)} kg</span>
                  </div>

                  <div className="p-4 bg-slate-50 rounded-2xl border border-slate-100 print:bg-slate-50 space-y-2">
                    <span className="text-[10px] font-black text-slate-600 uppercase tracking-widest block print-text-xs">Detalhamento dos Custos</span>
                    <div className="flex justify-between items-center text-xs">
                      <span className="font-bold text-slate-500">Juvenis/Alevinos:</span>
                      <span className="font-black text-slate-700">{formatCurrency(batchData.supplierInvoiceVal)}</span>
                    </div>
                    <div className="flex justify-between items-center text-xs">
                      <span className="font-bold text-slate-500">Consumo Ração:</span>
                      <span className="font-black text-amber-700">{formatCurrency(batchData.totalFeedCost)}</span>
                    </div>
                    <div className="flex justify-between items-center text-xs">
