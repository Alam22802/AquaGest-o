...
const HarvestManagement: React.FC<Props> = ({ state, onUpdate, currentUser }) => {
  ...
  const harvestedCageIds = useMemo(() => {
    if (!selectedBatchId) return new Set<string>();
    return new Set(
      (state.harvestLogs || [])
        .filter(h => h.batchId === selectedBatchId && h.id !== editingId)
        .map(h => h.cageId)
    );
  ...
  const handleSave = (e: React.FormEvent) => {
    ...
    if (editingId) {
      const updatedLogs = (state.harvestLogs || []).map(l => 
        l.id === editingId ? {
          ...
          initialFishCount: l.initialFishCount || initialFishCountToSave,
          date,
          updatedAt: Date.now()
        } : l
      );
      ...
      const newLog: HarvestLog = {
        ...
        averageWeight: averageWeight ? Number(averageWeight) : undefined,
        unitPrice: unitPrice ? Number(unitPrice) : undefined,
        initialFishCount: initialFishCountToSave,
        date,
    ...
    if (editingId) {
      ...
      // Check if there are more unharvested cages in the batch
      const harvestedCageIdsForBatch = new Set(
  ...
  const finalizedBatches = useMemo(() => {
    return (state.batches || []).filter(batch => {
      ...
      const activeUncoveredCages = state.cages.filter(c => 
        c.batchId === batch.id && c.status === 'Ocupada' && !harvestedCageIds.has(c.id)
      );
        
      return harvestedFish > 0 && activeUncoveredCages.length === 0;
    }).map(batch => {
