...
const BatchManagement: React.FC<Props> = ({ state, onUpdate, currentUser }) => {
  ...
  const removeBatch = (id: string) => {
    ...
    if (
      !confirm(
        "Deseja excluir este lote PERMANENTEMENTE? Os lançamentos de produção da fazenda (tratos, mortalidade, biometria e despescas) associados a ele serão apagados para liberar espaço. Os registros de peso e recepção da aba Frigorífico serão mantidos para controle histórico. Esta ação não pode ser desfeita.",
      )
    )
      return;
    ...
    const resolveLogBatchId = (
      ...
      const isValidForBatch = (b: Batch): boolean => {
        ...
        if (cleanDate) {
          ...
          if (logCageId) {
            const batchHarvests = sortedHarvestLogs.filter(h => h.cageId === logCageId && h.batchId === b.id);
      ...
      for (const b of state.batches || []) {
        ...
        const isCageInBatch = (b.cageIds || []).includes(logCageId) ||
          ...
          sortedHarvestLogs.some(h => h.cageId === logCageId && h.batchId === b.id);
