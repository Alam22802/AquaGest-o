...
const CapexManagement: React.FC<Props> = ({ state, onUpdate, currentUser }) => {
  ...
  const handleSaveInvoice = (e: React.FormEvent) => {
    ...
    const allInvoices = (state.capexInvoices || []).filter(i => i.projectId === invoiceForm.projectId && i.id !== editingInvoiceId);
    const newValue = Number(invoiceForm.value);

    // Direct invoices (not linked to PO) excluding the editing one
    const otherDirectInvoicesSum = allInvoices
      .filter(i => !i.purchaseOrderId)
      .reduce((acc, curr) => acc + curr.value, 0);

    const projectPOs = (state.capexPurchaseOrders || []).filter(p => p.projectId === invoiceForm.projectId);

    // Sum committed value by POs (taking into account invoices linked to them)
    const poCommittedValue = projectPOs.reduce((acc, po) => {
      const linkedInvoicesVal = allInvoices
        .filter(inv => inv.purchaseOrderId === po.id)
        .reduce((sum, inv) => sum + inv.value, 0);
      
      const isLinkedToCurrent = invoiceForm.purchaseOrderId === po.id;
      const totalInvoicesForPO = linkedInvoicesVal + (isLinkedToCurrent ? newValue : 0);
      return acc + Math.max(po.value, totalInvoicesForPO);
    }, 0);

    const isCurrentDirect = !invoiceForm.purchaseOrderId;
    const totalNewCommitment = otherDirectInvoicesSum + (isCurrentDirect ? newValue : 0) + poCommittedValue;

    if (totalNewCommitment > project.plannedValue) {
      alert(`Atenção: Saldo insuficiente no CAPEX do projeto! O lançamento da nota fiscal será efetuado, mas o projeto ficará com saldo excedente de R$ ${formatNumber(totalNewCommitment - project.plannedValue)}. Necessita de aporte/saldo.`);
    }
  ...
  const handleSavePurchaseOrder = (e: React.FormEvent) => {
    ...
    const allInvoices = (state.capexInvoices || []).filter(i => i.projectId === purchaseOrderForm.projectId);
    
    // Direct invoices not linked to any PO
    const directInvoicesSum = allInvoices
      .filter(i => !i.purchaseOrderId)
      .reduce((acc, curr) => acc + curr.value, 0);

    // Other POs for this project (excluding the one being edited, if editing)
    const otherPOs = (state.capexPurchaseOrders || []).filter(po => po.projectId === purchaseOrderForm.projectId && po.id !== editingPurchaseOrderId);

    // Committed value by other POs = sum of max(PO.value, linkedInvoicesVal)
    const otherPOsCommittedValue = otherPOs.reduce((acc, po) => {
      const linkedInvoicesVal = allInvoices
        .filter(inv => inv.purchaseOrderId === po.id)
        .reduce((sum, inv) => sum + inv.value, 0);
      return acc + Math.max(po.value, linkedInvoicesVal);
    }, 0);

    const consumedByOthers = directInvoicesSum + otherPOsCommittedValue;
    const availableBalance = project.plannedValue - consumedByOthers;

    const newValue = Number(purchaseOrderForm.value);

    // Invoices linked to the current PO being edited (if editing)
    const invoicesLinkedToThisPO = editingPurchaseOrderId
      ? allInvoices.filter(i => i.purchaseOrderId === editingPurchaseOrderId).reduce((sum, i) => sum + i.value, 0)
      : 0;

    const effectiveNewCommitment = Math.max(newValue, invoicesLinkedToThisPO);

    if (effectiveNewCommitment > availableBalance) {
