      ...
      if (prefix1.trim() === prefix2.trim() && startNum <= endNum) {
        ...
        for (let i = startNum; i <= endNum; i++) {
          ...
          newCages.push({
            id: generateId(),
            name: sequentialName,
            model: getInferredModel(Number(formData.length), Number(formData.width), Number(formData.depth), sequentialName),
            dimensions: {
              ...
              <div className="p-5 space-y-3">
                ...
                {cage.batchId && (
                  <div className="flex justify-between items-center bg-blue-50/80 p-2.5 rounded-xl border border-blue-100">
                    <span className="text-[10px] font-black text-blue-600 uppercase tracking-tight">Lote Vinculado</span>
                    <span className="text-xs font-black text-blue-800">
                      {(state.batches || []).find(b => b.id === cage.batchId)?.name || 'Lote Ativo'}
                      {cage.initialFishCount ? ` (${formatNumber(cage.initialFishCount, 0)} un)` : ''}
                    </span>
                  </div>
                )}
